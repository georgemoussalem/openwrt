...
linksys_bootconfig_toggle_primaryboot() {
	local partname=$1
	local tempfile
	local mtdidx
	...
	dd if=/dev/mtd"$mtdidx" of="$tempfile" 2>/dev/null
	[ $? -ne 0 ] || [ ! -f "$tempfile" ]&& {
		echo "failed to create a temp copy of /dev/mtd$mtdidx"
		return 1
	}

	toggle_bootconfig_primaryboot "$tempfile" "0:HLOS"
	[ $? -ne 0 ] && {
...
linksys_bootconfig_pre_upgrade() {
	...
	if [ -n "$UPGRADE_OPT_USE_CURR_PART" ]; then
		...
		linksys_bootconfig_toggle_primaryboot "0:BOOTCONFIG"
		linksys_bootconfig_toggle_primaryboot "0:BOOTCONFIG1"
	fi
